namespace Utility
{
    ...
    {
        public DockPanel _dpInner; // panel above BrowseList
        ...
        private int _lastSelChangedCount;
        private string _initialStatusString;
        private readonly IEnumerable<string> _arrColsToTotal;

        public BrowsePanel(IEnumerable query, int[] colWidths = null, IEnumerable<string> arrColsToTotal = null, bool ShowFilter = true)
        {
            this._arrColsToTotal = arrColsToTotal;
            try
            {
                _dpInner = new DockPanel();
                SetDock(_dpInner, Dock.Top);
                this.Children.Add(_dpInner);
                _query = query;
                ...
                if (ShowFilter)
                {
                    _dpInner.Children.Add(_listFilter);
                    DockPanel.SetDock(_listFilter, Dock.Top);
                }
                this.Children.Add(BrowseList);
                BrowseList.SelectionChanged += (o, e) =>
                {
                    DoCalcTotals(DoCalcEnum.Selchanged); // upon selection changed
                    this._lastSelChangedCount = BrowseList.SelectedItems.Count;
                };
                DoCalcTotals(DoCalcEnum.Initial); // initial
            }
        ...
        [Flags]
        internal enum DoCalcEnum
        {
            Initial,
            Selchanged, // occurs even when going from 1 to another witout multiselect
            FilterChanged
        }

        internal void DoCalcTotals(DoCalcEnum doCalcEnum)
        {
            try
            {
                if (doCalcEnum == DoCalcEnum.FilterChanged)
                {
                    _lastSelChangedCount = -1; // reset 
                }
                if (doCalcEnum == DoCalcEnum.Selchanged)
                {
                    if (_lastSelChangedCount != BrowseList.SelectedItems.Count)
                    {
                        doCalcEnum = DoCalcEnum.FilterChanged; // if the # selected items changed, treat like an update
                    }
                    else
                    {
                        if (_lastSelChangedCount == 1) // user went down arrow: totals didn't change and only 1 selected
                        {
                            return;
                        }
                    }
                ...
                var colNdx = 0;
                var totTxt = string.Empty;
                // iterate all (selected) items: can be expensive, so minimize: if selchanged, then only if # selitems > 1
                // if 1st time through, or filter changed, iterate all
                if (_arrColsToTotal != null || doCalcEnum == DoCalcEnum.Initial || doCalcEnum == DoCalcEnum.FilterChanged)
                {
                    IList items = null;
                    if (BrowseList.SelectedItems.Count > 1)
                    {
                        items = BrowseList.SelectedItems;
                        totTxt += $"#Selected = {items.Count:n0}";
                    }
                    else
                    {
                        if (doCalcEnum != DoCalcEnum.Selchanged) //don't iterate through all 1 million each time down-arrow is hit
                        {
                            items = BrowseList.Items;
                        }
                    }
                    if (items != null && _arrColsToTotal != null)
                    {
                        var totArray = new long[_arrColsToTotal.Count()];
                        foreach (var itm in items)
                        {
                            var tdescProps = TypeDescriptor.GetProperties(itm);
                            colNdx = 0;
                            foreach (var colname in _arrColsToTotal)
                            {
                                var val = tdescProps[colname].GetValue(itm);
                                totArray[colNdx++] += Convert.ToInt64(val);
                            }
                        }
                        colNdx = 0;
                        foreach (var colname in _arrColsToTotal)
                        {
                            totTxt += $" {colname}= {totArray[colNdx++]:n0}";
                        }
                    }
                }
                if (!string.IsNullOrEmpty(totTxt) || doCalcEnum != DoCalcEnum.Selchanged)
                {
                    var statStr = $"# items = {BrowseList.Items.Count:n0} {totTxt}".Trim();
                    if (string.IsNullOrEmpty(_initialStatusString))
                    {
                        _initialStatusString = statStr;
                    }
                    _listFilter._txtStatus.Text = statStr;
                }
            }
            catch (Exception)
            {
    ...
    internal class ListFilter : DockPanel
    {
        ...
        internal readonly TextBlock _txtStatus = new TextBlock();
        readonly BrowsePanel _browsePanel;
        internal ListFilter(BrowsePanel browse)
        {
            _browsePanel = browse;
        ...
        void On_BtnApply_Click(object o, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    _browsePanel.BrowseList.Items.Filter = null;
                }
                else
                {
                    _browsePanel.BrowseList.Items.Filter = itm =>
                    {
                        var fIsMatch = false;
                ...
                _browsePanel.DoCalcTotals(BrowsePanel.DoCalcEnum.FilterChanged); // upon filter changed
            }
            catch (Exception ex)
            {
    ...
    public class BrowseList : ListView
    {
        ...
        public GridViewColumn AddAColumn(string ColumnName, string bindingName, string toolTipHdr = null, string bindingObjectName = null, int colWidth = 0, int insertIndex = -1)
        {
            ...
            txtBlkFactory.SetValue(TextBlock.NameProperty, bindingName);
    ...
    public class MyValueConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, System.Globalization.CultureInfo culture)
        {
            if (null != value)
            {
                ...
                {
                    value = ((uint)value).ToString("n0"); // Add commas, like 1,000,000
                }
                ...
                {
                    value = ((ulong)value).ToString("n0"); // Add commas, like 1,000,000
                }
